...
class User(Base):
    ...
    rolle            = Column(SAEnum(Rolle, name="rolle"), default=Rolle.mitarbeiter, nullable=False)
    aktiv            = Column(Boolean, default=True)
    urlaubstage_jahr = Column(Integer, default=30)
    ...
    zeiteintraege   = relationship("Zeiteintrag", foreign_keys="Zeiteintrag.user_id", back_populates="user", cascade="all, delete-orphan")
    urlaubsantraege = relationship("Urlaubsantrag", foreign_keys="Urlaubsantrag.user_id", back_populates="user", cascade="all, delete-orphan")
...
class Baustelle(Base):
    ...
    status            = Column(SAEnum(BaustelleStatus, name="baustellestatus"), default=BaustelleStatus.aktiv)
...
class Urlaubsantrag(Base):
    ...
    id               = Column(Integer, primary_key=True, index=True)
    user_id          = Column(Integer, ForeignKey("users.id"), nullable=False)
    typ              = Column(SAEnum(AbwesenheitTyp, name="abwesenheittyp"), default=AbwesenheitTyp.urlaub)
    status           = Column(SAEnum(UrlaubStatus, name="urlaubstatus"), default=UrlaubStatus.beantragt)
    von_datum        = Column(Date, nullable=False)
    bis_datum        = Column(Date, nullable=False)
    arbeitstage      = Column(Integer, nullable=False)
...
class Regiezettel(Base):
    ...
    daten_snapshot         = Column(JSON, nullable=False)
    pdf_pfad               = Column(String(500), nullable=True)
